...
% - Version 1.0
% -------------------------------------------------------------------------
function [value map] = heterogeneityCalculateOldMap(volume, xStep, yStep, zStep, dis)
		...
		for j = 1:volumeDEM(2)
			for k = 1:volumeDEM(3)
				...
				if(ROIVolume(i,j,k)  == 1)
					...
					tempCovariance = tempCovariance/sum(weightVolume(:));
					map(i,j,k) = abs(tempCovariance) / (volume(i,j,k) -volumeAverage)^2;
				end
			end
		end
    end
